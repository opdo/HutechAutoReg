...
#cs Bao gồm các hàm chính
	_HttpRequest
	_HttpRequest_CreateDataFormSimple   Tạo nhanh biểu mẫu Data2Send đơn giản cho việc Upload (***Chưa hoàn thành)
	_HttpRequest_ClearCookies                  Xoá Cookie mà WinHttp đã sử dụng
...
Global $g___iUserAgent = 'Mozilla/5.0 (Windows NT 5.1) WinHttp/1.6.4.0 (WinHTTP/5.1) like Gecko'
...
Func _HttpRequest_SetOption($__Proxy = '', $__TimeOut = '', $__sHotKeyCancelReadWrite = '', $__Disable_Redirects = True)
	...
	$g___Disable_Redirects = ($__Disable_Redirects = False Or $__Proxy <> Default ? False : True)
...
Func _HttpRequest($iReturn, $sURL, $sDataToSend = '', $sCookie = $g___Cookies, $sReferrer = '', $sAdditional_Headers = '', $sOVerb = '', $ptCallBackFunc_ReadWrite = '', $ptCallBackFunc_AdditionalRequests = '')
	Local $aURL = StringRegExp($sURL, '^http(s?)://([^/]+)(.*?)$', 3)
	If @error Then Return SetError(1, '', '$sURL: Sai cấu trúc')
	;-------------------------------------------------
	Local $aReturn = StringRegExp($iReturn, '^([+-]?)(\d+)(\*?)$', 3)
	If @error Then Return SetError(2, '', '$iReturn: Sai cấu trúc')
	...
	;-------------------------------------------------
	If $g___iProxy Then
		$g___hOpen = _WinHttpOpen($g___iUserAgent, 3, $g___iProxy)
	Else
		If Not $g___hOpen Then $g___hOpen = _WinHttpOpen($g___iUserAgent)
	EndIf
	If $g___iTimeOut Then _WinHttpSetTimeouts($g___hOpen, Default, $g___iTimeOut, $g___iTimeOut, $g___iTimeOut)
	;-------------------------------------------------
	If $g___URLConnect <> $aURL[1] Then
		$g___URLConnect = $aURL[1]
		If $g___hConnect Then _WinHttpCloseHandle($g___hConnect)
		$g___hConnect = _WinHttpConnect($g___hOpen, $aURL[1], $g___PortFlag[0][$SSL])
	EndIf
	;-------------------------------------------------
	...
	;-------------------------------------------------
	If IsFunc($ptCallBackFunc_AdditionalRequests) Then $ptCallBackFunc_AdditionalRequests()
	;-------------------------------------------------
	If $sAdditional_Headers Then
		Local $aAddition = StringSplit($sAdditional_Headers, '|')
		For $i = 1 To $aAddition[0]
			If StringInStr($aAddition[$i], 'Content-Type:', 0, 1, 1, 15) Then
	...
	If $sDataToSend Then
		...
		Else
			...
			_WinHttpSendRequest($g___hRequest, 'Content-Type: multipart/form-data; boundary=' & StringTrimLeft($sBoundary, 2), Default, StringLen($sDataToSend))
			_WinHttpWriteData_Ex($g___hRequest, $sDataToSend, $ptCallBackFunc_ReadWrite)
			If @error Then $iError = @error
		EndIf
	Else
		_WinHttpSendRequest($g___hRequest)
	EndIf
	;-------------------------------------------------
	If $iError = 0 Then
		...
		Else
			...
			If $g___Authorization Then
				Local $iStatusCode = _WinHttpQueryHeaders($g___hRequest, $WINHTTP_QUERY_STATUS_CODE)
	...
	Switch $iReturn
		Case 1
			Return SetError($iError, $iReturn, $rData[0])
		Case 2, 3
			Return SetError($iError, $iReturn, $rData[1])
		Case Else
			Return SetError($iError, $iReturn, $rData)
	EndSwitch
EndFunc
...
Func _URLDecode($sData, $Escape_Character = '\u')
	...
	Next
	Return SetError((Not $sData) * 2, '', $sData)
EndFunc


Func _GetCookie($sHeader, $Excluded_Values = '')
	If $sHeader == '' Then SetError(1, '', '')
	...
	If @error Or Not IsArray($__aRH) Then Return SetError(2, '', '')
	Local $__sRH = '', $__uBound = UBound($__aRH)
	If Mod($__uBound, 2) Then Return SetError(3, '', '')
	For $i = $__uBound - 2 To 0 Step -2
		If $__aRH[$i] == '' Then ContinueLoop
		$__sRH &= $__aRH[$i] & '=' & $__aRH[$i + 1] & '; '
		For $k = 0 To $i Step 2
...
Func _GetLocation_Redirect($__sHeader)
	If $__sHeader == '' Then Return SetError(1, '', '')
...
Func _GetFileInfos($sFilePath)
	...
	FileClose($FileOpen)
	Local $aReturn[3] = [$sFileName[0], __WinHttpMIMEType($sFileName[0]), BinaryToString($sFileData)]
	Return $aReturn
EndFunc


Func _FileWrite_Test($sData)
	Local $FilePath = @TempDir & '\Test.html'
...
Func _B64Encode($binaryData, $iLinebreak = 64)
	If Not $binaryData Then Return SetError(1, '', '')
...
Func _B64Decode($base64Data)
	If Not $base64Data Then Return SetError(1, '', '')
